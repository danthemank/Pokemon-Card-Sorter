...
#from scservosdk.PortHandler import *
#from scservo_sdk.Input import *
#from scservo_sdk.Matcher import *
...
SCS_MOVING_STATUS_THRESHOLD = 200          # SCServo moving status threshold
SCS_MOVING_SPEED            = 5           # SCServo moving speed
...
def read_state():
    ...
    print(f"speed {scs_present_speed}, real speed {speed_real}")
...
while 1:
    ...
    while 1:
        ...
        error = abs(scs_goal_position[index] - int(speed_real))
        print (f"error {error}" )
        if not (abs(scs_goal_position[index] - speed_real) > SCS_MOVING_STATUS_THRESHOLD):
            break
